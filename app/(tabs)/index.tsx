...
export default function GameScreen() {
  ...
  const MAP_WIDTH = SCREEN_WIDTH * 3; // Enlarged map for horizontal scrolling
  ...
  const fireProjectile = () => {
    ...
    // For player 2, we need to adjust the angle calculation for the projectile
    const vx = Math.cos(angle) * power * (player.id === 1 ? 1 : -1);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.gameWrapper}>
        ...
        >
          <View style={[styles.gameCanvas, { width: MAP_WIDTH }]} ref={canvasRef}>
            ...
            {players.map(player => (
              <View
                ...
                style={{
                  position: 'absolute',
                  left: player.position - TANK_WIDTH / 2,
                  top: getTerrainYAtPosition(player.position) - TANK_HEIGHT - 5,
                  width: TANK_WIDTH,
                  height: TANK_HEIGHT,
              ...
              >
                {/* Fixed cannon aiming direction for both players */}
                <View
                  style={{
                    position: 'absolute',
                    left: player.id === 1 ? TANK_WIDTH / 2 : TANK_WIDTH / 2,
                    bottom: TANK_HEIGHT - 6,
                    width: 20,
                    height: 4,
                    backgroundColor: player.color,
                    borderRadius: 2,
                    transform: [
                      { translateX: player.id === 1 ? -3 : 3 },
                      { rotate: `${player.angle}deg` },
                      { translateX: player.id === 1 ? 3 : -3 },
                    ],
                    transformOrigin: player.id === 1 ? 'left center' : 'right center',
                  }}
                />
              </View>
            ))}
